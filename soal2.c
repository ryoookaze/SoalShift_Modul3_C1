...
int main() {
    ...
    pthread_create(tid+0, NULL, &startTurn, (void *) &pzip1);
    pthread_create(tid+1, NULL, &startTurn, (void *) &pzip2);
...
void* startTurn(void* argu) {
    ...
    int n_mine, mine, i;
    while(1) {
        while(*play1->status == play2->key) continue; // Player 2's turn
        ...
        for(i=0; i<4; i++) {
            ...
            // There's the mine :(
            if(play1->field & (1<<(mine-1))) play1->point++;
            else play2->point++;
        }
